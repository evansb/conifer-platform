...
// This gives something for the first and last characters to point to.
...
object SiteId {
  import util.Random
  val siteIdLength = 8
  def random: SiteId = SiteId(Random.alphanumeric.take(siteIdLength).mkString)
}
...
case class CharId(ns: SiteId, ng: ClockValue) extends Id {
  ...
  override def <(that: Id) = that match {
...
object WString {
  def empty(site: SiteId = SiteId.random): WString = WString(site, ClockValue(0))
}

// String representation
// Note there there is no `WChar` representation of Beginning and Ending:
// they are not included in the vector.
// The intention is for this data structure to be immutable: the `integrate` and `delete`
// operations produce new `WString` instances.
case class WString(site: SiteId,
                   nextTick: ClockValue,
  ...
  // The largest clock value for a site, when incremented,
  // is the starting clock value for that site.
